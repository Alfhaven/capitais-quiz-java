...
import quiz.repository.impl.CapitalRepositoryJPA;
import quiz.repository.impl.ContinenteRepositoryJPA;
import quiz.repository.impl.EstadoRepositoryJPA;
import quiz.repository.impl.PaisRepositoryJPA;
import quiz.repository.impl.UsuarioRepositoryJPA;
import quiz.service.AutenticacaoService;
import quiz.service.CadastroInvalidoException;
import quiz.service.ConteudoAdminService;

/**
 * Tela do administrador. Antes, cada botão "Salvar..." instanciava seu
 * próprio DAO e validava campos na mão; agora tudo isso foi extraído
 * para ConteudoAdminService (SRP) e a checagem "é admin?" usa
 * AutenticacaoService.isAdministrador (antes era um "admin".equals(...)
 * sensível a maiúsculas/minúsculas e que ignorava "administrador",
 * inconsistente com a checagem feita em LoginForm).
 */
public class AdminForm extends javax.swing.JFrame {
    ...
    private final ConteudoAdminService conteudoAdminService;
    private final AutenticacaoService autenticacaoService;

    public AdminForm(Usuario usuario) {
        this(usuario,
             new ConteudoAdminService(
                new ContinenteRepositoryJPA(),
                new PaisRepositoryJPA(),
                new EstadoRepositoryJPA(),
                new CapitalRepositoryJPA()),
             new AutenticacaoService(new UsuarioRepositoryJPA()));
    }

    public AdminForm(Usuario usuario, ConteudoAdminService conteudoAdminService,
                      AutenticacaoService autenticacaoService) {
    initComponents();
    this.usuarioLogado = usuario;
    this.conteudoAdminService = conteudoAdminService;
...
private void carregarContinentes() {
    ...
    for (Continente c : conteudoAdminService.listarContinentes()) {
...
private void carregarPaises() {
    ...
    for (Pais p : conteudoAdminService.listarPaises()) {
    ...
    private void jbttSalvarCapitaisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbttSalvarCapitaisActionPerformed
        ...
        TipoCapital tipo = TipoCapital.valueOf(jcbTipoCapital.getSelectedItem().toString());

        try {
            conteudoAdminService.salvarCapital(nome, pais, tipo);
    ...
    private void jbttSalvarEstadosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbttSalvarEstadosActionPerformed
        ...
        try {
            conteudoAdminService.salvarEstado(nome, pais);
    ...
    private void jbttSalvarPaisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbttSalvarPaisActionPerformed
        ...
        Continente continente = (Continente) jcbContinentePaís.getSelectedItem();

        try {
            conteudoAdminService.salvarPais(nome, continente);
    ...
    private void jbttSalvarContinenteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbttSalvarContinenteActionPerformed
        ...
        try {
            conteudoAdminService.salvarContinente(nome);
